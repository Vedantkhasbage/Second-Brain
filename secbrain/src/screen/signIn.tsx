...
export const SignIn = () => {
    ...
    function visitToDash(){
        navigate("/Dashboard")
    }

    async function handlesubmit(e){
        e.preventDefault();
          const response=await axios.post("http://localhost:2000/api/v1/signIn",{
                email:email,
                password:password
          })
    ...
    return <form onSubmit={handlesubmit} className="font-sour">
     <div className="h-screen w-full flex justify-center items-center">
        <div className="h-2/3  w-[400px] rounded-xl shadow-xl border-2	shadow-black">
            <div className="h-12 flex justify-center mt-4 mb-20">
                <h1 className="text-semibold mt-4 text-3xl">Welcome Back!!</h1>
            </div>
            ...
            {/* <button className=" h-[45px] w-96 rounded-3xl bg-green-300 mt-6 ">LogIn</button> */}
            <div  className="w-full flex items-center justify-center mt-8">
            <Button variants="primary" size="lg" text="LogIn"/>
            </div>
           </div>
